...
exports.getCategoriesPage = async (req, res, next) => {
  ...
  const totalItems = await Categories.find({
    categoriesDomain: itemSearch,
  }).countDocuments();
...
exports.getUserProductList = async (req, res, next) => {
  ...
  const totalItems = await Product.find({
    $or: [{ productDomain: itemSearch }, { categoriesName: itemSearch }],
  }).countDocuments();
...
exports.searchProductList = async (req, res, next) => {
  ...
  if (itemSearch1.length == 1) {
    totalItems = await Product.find({
      $or: [
        { categoriesDomain: { $regex: "^" + itemSearch1[0], $options: "i" } },
    ...
    if (totalItems <= 0) {
      totalItems = await Product.find({
        $or: [
          { searchKeywordFirst: { $regex: itemSearch, $options: "i" } },
